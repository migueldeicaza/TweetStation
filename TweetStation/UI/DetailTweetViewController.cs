	...
	public class DetailTweetViewController : DialogViewController {
		...
		public DetailTweetViewController (Tweet tweet) : base (UITableViewStyle.Grouped, null, true)
		{
			...
			Root = new RootElement (tweet.Screename){
				...
				replySection,
				new Section () {
					new TimelineElement (tweet.Screename, Locale.GetText ("User's timeline"), "http://api.twitter.com/1/statuses/user_timeline.json?skip_user=true&id=" + tweet.UserId, User.FromId (tweet.UserId))
				}
			};
		}
